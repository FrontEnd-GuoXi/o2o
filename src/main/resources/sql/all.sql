...
ALTER TABLE `wechat_auth` ADD CONSTRAINT `unique_open_id` UNIQUE(`open_id`);
SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'wechat_auth' AND REFERENCED_TABLE_NAME IS NOT NULL;
# 'wechat_auth_ibfk_1'
ALTER TABLE `wechat_auth` DROP FOREIGN KEY `foreign_user_id`;
ALTER TABLE `wechat_auth` ADD CONSTRAINT `fk_wechat_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES tb_person_info(`user_id`);
...
SELECT CONSTRAINT_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'local_auth' AND REFERENCED_TABLE_NAME IS NOT NULL;
# 'local_auth_ibfk_1'
ALTER TABLE `local_auth` DROP FOREIGN KEY `local_auth_ibfk_1`;
ALTER TABLE `local_auth` ADD CONSTRAINT `fk_local_auth_user_id` FOREIGN KEY (`user_id`) REFERENCES tb_person_info(`user_id`);
